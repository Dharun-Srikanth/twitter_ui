...
class DBHelper {
  ...
  static const String LANGUAGE_TABLE = "Language";
  ...
  Future _onCreate(Database db, int version) async {
    ...
    await db.execute("CREATE TABLE $LANGUAGE_TABLE"
        "(id INTEGER PRIMARY KEY AUTOINCREMENT,"
        "language TEXT)");
  }
  ...
  Future<void> addLanguage() async {
    ...
    await dbClient!.rawInsert(
      "INSERT INTO $LANGUAGE_TABLE (language)"
        " SELECT 'en' "
        "WHERE NOT EXISTS (SELECT 1 FROM $LANGUAGE_TABLE WHERE language='en')"
    );
  }

  Future<void> updateLanguage(String lang) async {
    var dbClient = await database;
    await dbClient!.rawUpdate("UPDATE $LANGUAGE_TABLE "
        "SET language=?"
        "WHERE id=?",
        [lang, 1]);
  }
  ...
  Future<LanguageModel> getLang() async {
    ...
    return LanguageModel.fromJson(result.first);
  }
